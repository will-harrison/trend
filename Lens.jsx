...
let Lens = React.createClass({
  render() {
    ...
    let weeklySales = _(items)
      ...
      .map(i => i.weeklySales)
      .flatten()
      .sortBy(i => new Date(i.weekEndDate))
      .groupBy(i => i.weekEndDate)
      ...
      .reduce((memo, val, index) => {
        ...
        memo.push({
          ...
          salesUnits: _.sum(val, i => i.netSalesUnits)
        });
        return memo;
      }, [])

      ;
    console.log(_(items).map(i => i.weeklySales).flatten().map(i => i.salesUnits).value())
    return (
      <div className="uk-container uk-container-center">
        ...
        <div className="uk-grid uk-grid-width-1-2">
          ...
          <Financial items={items} STDSales={_.sum(weeklySales.salesUnits)} />
        </div>
      </div>
    );
  },
...
let Chart = React.createClass({
  render() {
    ...
    let labels = _.map(weeklySales, i=>i.weekEndingDate);
    // console.log(weeklySales);
    return (
      <div className="uk-width-7-10">
        {weeklySales.map(i => i.weekEndingDate + ": " + i.salesUnits + " |  ")}
      </div>
    );
  },
});
...
let Financials = React.createClass({
  render() {
    let { financials, STDSales } = this.props;
    return (
...
let Financial = React.createClass({
  render() {
    let { items, STDSales } = this.props;
    // let STDSales = _.sum(items, i => i.weeklySales.salesUnits);
    let WTDSales = _.sum(items, i => i.WTDSales);
    let planAPS = _.sum(items, i => i.planAPS) / _.size(items);
    let avgRetail = _.sum(items, i => i.retailPrice) / _.size(items);
    let avgCost = _.sum(items, i => i.unitCost) / _.size(items);
    let grossMarginDollars = avgRetail - avgCost;
    let grossMarginPercent = grossMarginDollars / avgRetail;
    console.log(STDSales);
    return (
...
let Panel = React.createClass({
  render() {
    let { title, body, format } = this.props;
    ...
    return (
      <div className="uk-grid uk-panel uk-panel-divider">
        <div className="uk-width-3-4  uk-panel-header">
          ...
          <h3 style={{"marginTop": "-15px"}}>{numeral(body).format(format)}</h3>
        </div>
